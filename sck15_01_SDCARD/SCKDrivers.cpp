...
void SCKDriver::getSHT(uint32_t* __Temperature, uint32_t* __Humidity)
   {
        *__Temperature = (-46.85 + (175.72*(readSHT(0xE3)/65536.0)))*100;
        *__Humidity    = (-6 + (125*(readSHT(0xE5)/65536.0)))*100; 
...
int SCKDriver::levelRead(boolean state) {
    return 2*(3300/1023.)*average(state);
}
...
void SCKDriver::chargerMode(boolean state1, boolean state2)
  {
//    digitalWrite(P_SEL, state1);  //Power source selection input. High indicates a USB host source and Low indicates an adapter source.
//    digitalWrite(OTG, state2);
  }
...
float SCKDriver::readResistor(byte resistor ) {
 ...
 void SCKDriver::writeADC(byte address, byte value)
  {
    writeI2C(ADC_DIR, address, value);
  }
  
 byte SCKDriver::readADC(byte channel)
  {
    byte rdata = 0xFF;
    byte  data = 0x0000;
    Wire.beginTransmission(ADC_DIR);
    Wire.write(channel);
    Wire.endTransmission();
    Wire.requestFrom(ADC_DIR,1);
    unsigned long time = millis();
    while (!Wire.available()) if ((millis() - time)>500) return 0x00;
    data = Wire.read(); 
    return data;
